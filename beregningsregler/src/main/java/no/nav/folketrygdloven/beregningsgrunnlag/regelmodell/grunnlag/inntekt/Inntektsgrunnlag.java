...
public class Inntektsgrunnlag {
    ...
    public DagsatsOgBeregnetPrÅr regnUtSnittInntektForDPellerAAP(AktivitetStatus aktivitetStatus, LocalDate stp, boolean medUtbetalingsfaktor) {
        if (aktivitetStatus.erDPFraYtelse()) {
            var dagpengerFraYtelseVedtak = getDagpengerFraYtelseVedtak();
            if (dagpengerFraYtelseVedtak.isPresent()) {
                ...
                return new DagsatsOgBeregnetPrÅr(dagsats, beregnetPrÅr);
            }
        }
        var relevanteInntekter = getInntekterForEnkeltdagerFørStpForDPellerAAP(stp);
    ...
    private Optional<Periodeinntekt> getDagpengerFraYtelseVedtak() {
        return getSistePeriodeinntekterMedType(Inntektskilde.YTELSE_VEDTAK).stream()
            .filter(i -> i.getInntektskategori().equals(Inntektskategori.DAGPENGER))
            .findFirst();
    }

    private List<Periodeinntekt> getInntekterForEnkeltdagerFørStpForDPellerAAP(LocalDate stp) {
        return getPeriodeinntekter().stream()
