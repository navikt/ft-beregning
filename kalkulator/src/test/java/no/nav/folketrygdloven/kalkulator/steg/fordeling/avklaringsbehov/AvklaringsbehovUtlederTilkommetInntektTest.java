...
class AvklaringsbehovUtlederTilkommetInntektTest {
	...
	@Test
	void skal_finne_tilkommet_dagpengeandel_dersom_en_arbeidstakerandel_fra_start_med_overlapp_til_dagpenger() {
		...
		var tilkommetAktivitet = finnTilkomneAndeler(periode, List.of(yrkesaktivitet),
				List.of(Intervall.fraOgMedTilOgMed(STP.plusDays(2), STP.plusDays(15))),
				List.of(),
	...
	private Set<StatusOgArbeidsgiver> finnTilkomneAndeler(Intervall periode,
		...
		return finnTilkomneAndeler(periode, yrkesaktiviteter, List.of(), inntekter, andelerFraStart, utbetalingsgradGrunnlag, skjæringstidspunkt);
	}

	private Set<StatusOgArbeidsgiver> finnTilkomneAndeler(Intervall periode,
	                                                      List<YrkesaktivitetDto> yrkesaktiviteter,
	                                                      List<Intervall> dagpengePerioder,
	                                                      List<InntektDtoBuilder> inntekter,
	                                                      List<BeregningsgrunnlagPrStatusOgAndelDto> andelerFraStart,
	                                                      PleiepengerSyktBarnGrunnlag utbetalingsgradGrunnlag,
	                                                      LocalDate skjæringstidspunkt) {
		...
		if (!dagpengePerioder.isEmpty()) {
			var fom = dagpengePerioder.stream().map(Intervall::getFomDato).min(Comparator.naturalOrder()).orElseThrow();
			var tom = dagpengePerioder.stream().map(Intervall::getTomDato).max(Comparator.naturalOrder()).orElseThrow();
			var aktørYtelse = InntektArbeidYtelseAggregatBuilder.AktørYtelseBuilder.oppdatere(Optional.empty());
			var ytelseDtoBuilder = YtelseDtoBuilder.ny().medYtelseType(YtelseType.DAGPENGER).medPeriode(Intervall.fraOgMedTilOgMed(fom, tom));
			dagpengePerioder.forEach(p ->
					ytelseDtoBuilder.leggTilYtelseAnvist(YtelseAnvistDtoBuilder.ny().medAnvistPeriode(p)
							.medBeløp(Beløp.fra(1000))
							.medDagsats(Beløp.fra(1000))
							.medUtbetalingsgradProsent(Stillingsprosent.fra(200)).build()));
			aktørYtelse.leggTilYtelse(
					ytelseDtoBuilder
			);
