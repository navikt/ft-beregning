...
class FinnBesteMåneder extends LeafSpecification<BesteberegningRegelmodell> {
    ...
    static final String BESKRIVELSE = "Finn 6 beste måneder av de 10 siste";
    ...
    public Evaluation evaluate(BesteberegningRegelmodell regelmodell) {
	    ...
	    månedsgrunnlagListe.sort(Comparator.comparing(BeregnetMånedsgrunnlag::finnSum).reversed());
	...
	private List<BeregnetMånedsgrunnlag> lagInntekterPrMåned(BesteberegningRegelmodell regelmodell, Map<String, Object> resultater) {
		...
		BigDecimal gjenommsnittligPGI = FinnGjennomsnittligPGI.finnGjennomsnittligPGI(skjæringstidspunktOpptjening,
		...
		LocalDate førsteFom = skjæringstidspunktOpptjening.minusMonths(11).withDayOfMonth(1);
		LocalDate sisteFom = skjæringstidspunktOpptjening.withDayOfMonth(1);
	...
	private BeregnetMånedsgrunnlag lagGrunnlagForMåned(List<Periodeinntekt> inntekter, List<Periode> perioderMedNæringsvirksomhet, BigDecimal gjenommsnittligPGI, LocalDate fom) {
		...
		inntekter.stream()
				.filter(p -> Periode.of(p.getFom(), p.getTom()).overlapper(periode))
	...
	private Inntekt mapTilInntekt(Periodeinntekt periodeinntekt) {
		if (periodeinntekt.getArbeidsgiver().isPresent()) {
			...
			return new Inntekt(aktivitetNøkkel, periodeinntekt.getInntekt().multiply(inntektPeriodeType.getAntallPrÅr()));
		}
	}
