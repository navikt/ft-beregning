...
public class FastsettSammenligningsgrunnlagForSN extends LeafSpecification<BeregningsgrunnlagPeriode> {
    ...
    private static final BigDecimal OPPJUSTERINGS_FAKTOR_AAP = BigDecimal.valueOf(0.66);
    private static final BigDecimal OPPJUSTERINGS_FAKTOR_DP = BigDecimal.valueOf(0.624);
    ...
    private SammenligningsGrunnlag opprettSammenligningsgrunnlag(BeregningsgrunnlagPeriode grunnlag, Periodeinntekt oppgittInntekt) {
        ...
        BigDecimal oppjustertAAP = bruttoAAP.divide(OPPJUSTERINGS_FAKTOR_AAP, 10, RoundingMode.HALF_UP);
        BigDecimal oppjustertDP = bruttoDP.divide(OPPJUSTERINGS_FAKTOR_DP, 10, RoundingMode.HALF_UP);
        ...
        BigDecimal sammenligningInntekt = oppgittÅrsInntekt.add(bruttoATFL).add(oppjustertAAP).add(oppjustertDP);
    ...
    private Map<String, Object> gjørRegelsporing(BeregningsgrunnlagPeriode grunnlag, SammenligningsGrunnlag sammenligningsGrunnlag, Periodeinntekt oppgittInntekt) {
        ...
        String oppjustertString = "oppjustert";
        if (bgATFL != null) {
            String status = "ATFL";
        ...
        if (bgAAP != null) {
            ...
            resultater.put(oppjustertString + status, bgAAP.getBeregnetPrÅr().divide(OPPJUSTERINGS_FAKTOR_AAP, 2, RoundingMode.HALF_UP));
        }
        if (bgDP != null) {
            String status = "DP";
            resultater.put(bruttoString + status, bgDP.getBeregnetPrÅr());
            resultater.put(oppjustertString + status, bgDP.getBeregnetPrÅr().divide(OPPJUSTERINGS_FAKTOR_DP, 2, RoundingMode.HALF_UP));
        }
